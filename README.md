...
| 用户操作 | 发生了什么 | 用户看到什么 |
|---------|-----------|-------------|
| 点「传图」选 2 张壁纸 | 逐张分析（约 10~40 秒/张） | 按钮 `1/2 分析中…` → `2/2 分析中…` → 旁边出现「已传 2 张」 |
| 鼠标悬停「已传 2 张」 | — | 淡入列表：`1. 倒影-山水-山脉 ✕`、`2. 天空-山川-景观 ✕` |
| 点 ✕ 删掉第 2 张 | 文件删除、分析移除 | 列表剩 1 张 |
| 输入"这两张图是啥"并发送 | 分析自动随消息注入 | 模型逐张回答，无需你再解释 |
